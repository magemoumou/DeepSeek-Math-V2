...
flowchart TD
    ...
    %% 关键模型推理调用 - 证明生成
    ExecGenCmd --> ModelGenCall[模型推理调用 #1<br/>AsyncOpenAI.chat.completions.create]:::modelInfer
    ModelGenCall --> GenAsyncProcess[异步批量处理<br/>温度:1.0, max_tokens:128K]:::modelInfer
    
    GenAsyncProcess --> SaveGenResult[保存生成结果]:::storage
    
    %% 第二轮：证明验证
    SaveGenResult --> PrepVerifInput[准备验证输入数据]:::dataProcess
    ...
    %% 关键模型推理调用 - 证明验证
    ExecVerifCmd --> ModelVerifCall[模型推理调用 #2<br/>AsyncOpenAI.chat.completions.create]:::modelInfer
    ModelVerifCall --> VerifAsyncProcess[异步批量处理<br/>温度:1.0, max_tokens:64K]:::modelInfer
    
    VerifAsyncProcess --> SaveVerifResult[保存验证结果]:::storage
    
    %% 第三轮：元验证
    SaveVerifResult --> CheckScore{验证评分<br/>小于0.75?}:::decision
    ...
    CreateMetaCmd --> ExecMetaCmd[执行元验证命令]:::codeControl
    
    %% 关键模型推理调用 - 元验证
    ExecMetaCmd --> ModelMetaCall[模型推理调用 #3<br/>AsyncOpenAI.chat.completions.create]:::modelInfer
    ModelMetaCall --> MetaAsyncProcess[异步批量处理<br/>温度:1.0, max_tokens:64K]:::modelInfer
    
    MetaAsyncProcess --> SaveMetaResult[保存元验证结果]:::storage
    
    %% 多轮迭代控制
    SaveMetaResult --> UpdatePool[更新证明池数据]:::storage
    SkipMeta --> UpdatePool
    
    UpdatePool --> CheckRound{是否达到<br/>最大轮次?}:::decision
    CheckRound -->|否| IncrementRound[轮次计数器+1]:::codeControl
    IncrementRound --> PrepVerifInput
    
    CheckRound -->|是| FinalOutput[输出最终结果]:::storage
